...
def repair_pdb(pdb_path: Path, work_dir: Path = None) -> Path:
    """
    Run FoldX RepairPDB on a structure.
    FoldX must run from the directory containing the PDB file,
    so we copy the PDB into work_dir (or FOLDX_WORK_DIR) and run from there.
    Returns path to the repaired PDB (cached if already exists).
    """
    wdir = work_dir if work_dir is not None else FOLDX_WORK_DIR
    wdir.mkdir(parents=True, exist_ok=True)
    repaired = wdir / f"{pdb_path.stem}_Repair.pdb"
    if repaired.exists():
        return repaired
    ...
    rotabase_dst = wdir / "rotabase.txt"
...
def predict_ddg(repaired_pdb: Path, mut_str: str, work_dir: Path = None) -> float | None:
    """
    ...
    """
    wdir = work_dir if work_dir is not None else FOLDX_WORK_DIR
    mut_dir = wdir / mut_str
    mut_dir.mkdir(parents=True, exist_ok=True)

    mutant_file = wdir / "individual_list.txt"
    ...
    avg_file = wdir / f"Average_{stem}.fxout"
    raw_file = wdir / f"Raw_{stem}.fxout"

    fxout = avg_file if avg_file.exists() else raw_file if raw_file.exists() else None
    if fxout is None:
        all_fxout = [f.name for f in wdir.glob("*.fxout")]
    ...
    for f in wdir.glob(f"*_{stem}.fxout"):
