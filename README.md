...
├── spatial_context.py     # Positional context features
├── tests/
│   └── find_prots.py      # Screen SKEMPI for best candidate complexes
├── ML/
...
│   ├── energy_terms.py    # Parse FoldX energy term columns into parquet
│   ├── generate_features.py  # Base feature matrix builder (standalone inspection)
│   ├── assemble_features.py  # Extended feature matrix: base + energy terms + scan
│   └── classifier.py      # RF classifier with LOPO and per-structure CV
├── protein_cv/            # CV utilities and sibling-DDG features (used in notebooks)
├── notebooks/
│   └── test3.ipynb        # Phase 3: positional context analysis
├── data/
...
└── results/
    ...
    ├── energy_terms.parquet                       # FoldX energy term columns
    ├── feature_matrix_extended_<target>.parquet   # Full feature matrix for notebook
    └── figures/
```
...
| File | Contents |
|---|---|
| `results/energy_terms.parquet` | FoldX energy term columns (16 terms per mutation) |
| `results/feature_matrix_extended_<target>.parquet` | Full feature matrix (base + energy terms + scan) |
| `results/figures/` | All plots |

---

## Phase 3 — Positional Context (Notebook)
...
Reads `results/feature_matrix_extended_DDG.parquet` produced in Phase 2. Uses the `protein_cv` package for sibling-DDG features and position-grouped CV. Follow the cells in order; intermediate outputs are displayed inline.
